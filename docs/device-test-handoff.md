# 独立フォントパッチの実機検証に向けた引き継ぎ（旧 shell / awk 版）

この資料は、2026-09-18 に作成した、Python 移行前の実機検証の依頼記録です。以下の作業ツリー・端末状態・ZIP ハッシュ・未実装項目は、引き継ぎ作成時点の情報です。現在の導入手順は [README](../README.md)、移行後の結果は [Python 版の実機検証記録](python-device-validation.md)と [Serif・Mono の実機検証記録](serif-mono-device-validation.md)を参照してください。

検証の目的は、ホストの自動テストでは確認できない TWRP 上の導入処理、Android の起動・フォント選択・描画、各パッチの個別復元を確かめることです。作成時点では、ホスト側の実装・ビルド・自動テストを完了していました。以下に当時の前提と検証手順を記載します。
...
資料作成時に実ファイルのハッシュを再計算し、当時の `dist/SHA256SUMS.txt` と一致することを確認しました。検証に使用する際は、PC 側と転送後の端末側でも照合してください。実装を修正・再ビルドした場合は、新しい成果物のハッシュを記録し、上記の旧ハッシュと区別してください。
