...
証跡: `build/device-validation/20260918-python/`（Git 管理対象外）。`tested-source.zip` にコード、`tested-source-sha256.json` にハッシュ、`worktree.patch` / `git-status.txt` / `head.txt` に未コミット作業ツリーの記録を保存しました。各段階の TWRP ログ、XML 本体、フォントハッシュ、属性、Android boot properties と logcat を保存しています。
...
上記4 ZIP のみが本記録の対象です。`verification.json` の `device_tested: false` はビルド時の静的報告のまま保持し、実機の結果をこの文書と証跡で別に記録しています。
...
同じ TWRP 起動中に CJK / 欧文それぞれ install を2回、restore を2回実行（計8適用）。すべて成功し、`/tmp` のランタイム・作業領域を再利用できました。証跡は `repeat-cjk-1`〜`4`、`repeat-latin-1`〜`4` です。

通常の成功後の再適用と、ホストでの prepare 失敗後のステージング拒否を確認しています。実機の電源断・容量枯渇・実ファイル破壊による試験は実施していません。

## 起動待ちとログ

初回の Android → TWRP 再起動後、`ro.twrp.version` を取得できても `twrp remountrw` が終了コード255になりました。ZIP 適用前の制御受付準備の問題で、その後の `remountrw` は成功。補助スクリプトを完了応答まで待つよう修正しました。今回の Python 版 ZIP 自体の適用失敗はありません。
...
表示確認はユーザーが手動で行う方針です。初期に採取した画像は描画合格の根拠にしていません。設定画面の起動要求が error code 101 で拒否された記録もあり、画像ファイル名が settings であっても設定画面を表示できたとは扱いません。検証 APK は追加していません。
...
ユーザー指定に従い、検証で S へ戻した後に CJK と欧文を導入し、**B（CJK＋欧文）で Android 起動済み**です。最終証跡は `final-cjk-install` と `final-both-install`。
