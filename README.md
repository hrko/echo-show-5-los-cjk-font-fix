...
Echo Show の LineageOS 18.1（Android 11）に、可変フォントを導入する TWRP ZIP を生成します。

本パッチを適用すると、次のように文字の表示を改善できます。

- **日本語の太字を美しく表示**：標準の日本語フォントはレギュラーのみで、太字は疑似的に生成されます。CJK パッチでは太字用の字形を使い、自然な太字で表示できます。
- **細字から極太字まで対応**：CJK パッチでは、日本語もアプリや Web ページが指定するウェイト（文字の太さ）に応じて表示できます。
- **Pixel 風の見た目に**：欧文（latin）パッチで Google Sans Flex を使うと、英数字などの表示を最近の Pixel 端末の雰囲気に近づけられます。

**各パッチの同じ ZIP を、以下の3機種の対象 ROM で使用できます。**
